Namespace('brook').define(function(ns){var VERSION="0.01";var k=function(next,val){next(val);};var lift=function(f){return(f instanceof Promise)?f:new Promise(f);};var Promise=function(next,errorHandler){this.next=next||k;if(errorHandler)
this.setErrorHandler(errorHandler);};(function(proto){proto.concat=function(after){var _before=this;var next=function(n,val){_before.subscribe(after.ready(n),val);};return new Promise(next);};proto.bind=function(){var r=this;for(var i=0,l=arguments.length;i<l;i++){r=r.concat(lift(arguments[i]));}
return r;};proto.ready=function(n){var promise=this;return function(val){promise.subscribe(n,val);};};proto.run=function(val){this.subscribe(undefined,val);};var empty=function(){};proto.subscribe=function(next,val){var next=next||empty;if(!this.errorHandler)
return this.next(next,val);try{this.next(next,val);}
catch(e){this.onError(e);}};proto.forEach=proto.subscribe;proto.setErrorHandler=function(f){this.errorHandler=lift(f);};proto.onError=function(e){(this.errorHandler||new Promise).run(e);};})(Promise.prototype);var promise=function(next,errorHandler){return new Promise(next,errorHandler)};ns.provide({promise:promise,VERSION:VERSION});});Namespace('brook.util')
.use('brook promise')
